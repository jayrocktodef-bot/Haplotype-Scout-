...
with open("src/data/snpDatabase.ts") as f:
    snp_code = f.read()

new_ydna_entries = """  // ── Sub-Saharan African & Diaspora Y-DNA (E1b1a Deep Subclades)
...
# Insert new Y-DNA SNPs before MTDNA SNPS comment
marker_mt_hdr = "  // ==========================================\n  // MTDNA SNPS"
if marker_mt_hdr in snp_code:
    snp_code = snp_code.replace(marker_mt_hdr, new_ydna_entries + "\n" + marker_mt_hdr)
else:
    print("Warning: marker_mt_hdr not found in snpDatabase.ts")

# Insert new mtDNA SNPs before ending ];
last_bracket = "\n];"
snp_code = snp_code.rstrip()
if snp_code.endswith("];"):
    snp_code = snp_code[:-2] + new_mtdna_entries + "\n];\n"

with open("src/data/snpDatabase.ts", "w") as f:
    f.write(snp_code)

print("Updated src/data/snpDatabase.ts successfully.")
